...
public class SubscriptionService {
	...
	public Page<SubscriptionDTO> searchSubscriptions(Pageable pageable) {
		return subscriptionRepository.findAll(pageable != null ? pageable : Pageable.unpaged()).map(subscriptionMapper::toDto);
	}
	...
	public Long createSubscription(@NotNull @Valid SubscriptionRequestDTO subscriptionRequestDTO) {
		...
		notifyNewSubscription(subscription);

		return subscription != null ? subscription.getId() : null;
	}

	private void notifyNewSubscription(Subscription subscription) {
		...
		emailRequestDTO.setEmailContent("<div>You are now subscrived for campaing " + subscription.getNewsletter().getCampaignYear() + " newsletter</div>");
		
		emailService.sendEmail(emailRequestDTO);
	}

	public void updateSubscription(@NotNull Long id, @NotNull @Valid SubscriptionRequestDTO subscriptionRequestDTO) {
		Optional<NewsLetter> newsLetter = newsLetterRepository.findById(subscriptionRequestDTO.getNewsLetterId());
		if (!newsLetter.isPresent()) {
			throw new BadRequestException("The Newsletter doesnt exist");
		}
		...
		notifySubscriptionModification(subscription);
	}

	private void notifySubscriptionModification(Subscription subscription) {
		EmailRequestDTO emailRequestDTO = new EmailRequestDTO();
		emailRequestDTO.setFromEmail("no-repply@adidas.com");
		emailRequestDTO.setDestEmail(subscription.getEmail());
		emailRequestDTO.setEmailSubject("Subscription modified");
		emailRequestDTO.setEmailContent("<div>Your subscription for campaign " + subscription.getNewsletter().getCampaignYear() + " newsletter has been modified</div>");
		
		emailService.sendEmail(emailRequestDTO);
	}

	public void removeSubscription(@NotNull Long id) {
		Optional<Subscription> currentSubscription = subscriptionRepository.findById(id);
		if (!currentSubscription.isPresent()) {
			throw new NotFoundException("Subscription doesnt exist");
		}
